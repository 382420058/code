...
if(isset($_POST['bdpd'])){
	...
	if(isset($_POST['id'])){
		if(empty($_POST["remarks"])){exit("<script>javascript:alert('对不起，备注信息不完，无法匹配!');self.location=document.referrer;</script>");}
	...
	$sql = "select id,remarks,content from chzb_epg where remarks != ''";
	$result=mysqli_query($GLOBALS['conn'],$sql);
	if (!mysqli_num_rows($result)) {
		mysqli_free_result($result);
...
if ($_GET["act"]=="add") {
	...
	//新加EPG数据
	mysqli_query($GLOBALS['conn'],"insert into chzb_epg (name,remarks) values ('".$epg_name."','".$remarks."')");
...
<tr>
	<table width="900" align="center" border="1" bordercolor="#a0c6e5" style="border-collapse:collapse;">
		...
		EPG名称:<input type="text" name="name" value="<?php echo $na;?>">&nbsp;&nbsp;备注:<input type="text" name="remarks" value="<?php echo $bz;?>"><input type="hidden" name="id" value="<?php echo $id;?>">&nbsp;&nbsp;<input type="submit" name="bdpd" value="搜索绑定频道" onclick="return confirm('自动绑定频道列表后,如果不准确请手动修改!!!')">&nbsp;&nbsp;<input type="submit" name="qkbd" value="清空绑定" onclick="return confirm('确定要清空绑定的频道列表吗？')">
		</tr></td><tr><td>
